...
import { X, AlertCircle, Copy, Check, Terminal, Cookie } from "lucide-react";
import { CSSBUY_SCRAPER_SCRIPT } from "@/lib/cssbuyScraperScript";

interface SyncDialogProps {
  open: boolean;
  onClose: () => void;
  onSyncCookie: (cookie: string) => Promise<unknown>;
...
export default function SyncDialog({
  ...
  const [mode, setMode] = useState<"cookie" | "script">("cookie");
  ...
  const handleCookieSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onSyncCookie(cookie.trim());
      setCookie("");
      onClose();
    } catch (err) {
        ...
        {mode === "cookie" ? (
          <form onSubmit={handleCookieSubmit} className="p-4 space-y-4">
            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-xs text-amber-400 flex items-start gap-2">
              ...
              <p>
                Pegá el valor completo de la cookie de sesión de CSSBuy. Si CSSBuy valida la IP, esto puede fallar;
                usá la opción "Script consola" como alternativa.
              </p>
            </div>
          ...
          <form onSubmit={handleJsonSubmit} className="p-4 space-y-5">
            <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg text-xs text-blue-400 flex items-start gap-2">
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <p>
                Útil si CSSBuy rechaza la cookie por validación de IP. El script corre en tu navegador con tu IP real.
              </p>
            </div>
