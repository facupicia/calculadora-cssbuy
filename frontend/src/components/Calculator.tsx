...
import {
  Plus, Trash2, Save, Download, RotateCcw, ArrowRight,
  AlertTriangle, Info, ShoppingBag, Plane, Shield, DollarSign
} from 'lucide-react';
import { Product, FxRates, ShipmentCosts, AduanaConfig, CalculationResult } from '../types';
import { calcularTodo, fmtUSD, fmtARS, fmtPct, uid } from '../lib/utils';

interface CalculatorProps {
  orders: import('../types').CssbuyOrder[];
}

export default function Calculator({ orders }: CalculatorProps) {
  // Estado
  const [fx, setFx] = useState<FxRates>({ blue: 0, oficial: 0, mep: 0, cny: 7.20 });
  const [envio, setEnvio] = useState<ShipmentCosts>({ freightCNY: 0, serviceCNY: 0, recargaPct: 0.03, recargaFijo: 0.30, platformFee: 0.35, markup: 2.0 });
  const [lineaEnvio, setLineaEnvio] = useState<string>('custom');
  const [tarifaPorGramo, setTarifaPorGramo] = useState<number>(0);
  const [aduana, setAduana] = useState<AduanaConfig>({ dentroFranquicia: false, enviosAnio: 0, ivaPct: 0.21, iibbPct: 0.03, valorDeclaradoUSD: null });
  ...
  // Calcular freight automáticamente según peso total y tarifa seleccionada
  ...
  // Si la línea no es manual y hay peso, actualizar freight
  ...
  const addProducto = useCallback(() => {
    setProductos(prev => [...prev, { id: uid(), nombre: '', precioCNY: 0, envioLocalCNY: 0, envioChinaCNY: 0, pesoG: 0, cantidad: 1, precioVentaUSD: 0, link: '', imgURL: '' }]);
  }, []);
  ...
  const importFromOrder = useCallback((order: import('../types').CssbuyOrder) => {
  ...
  const guardarCotizacion = useCallback(() => {
    if (resultados.productosUSDTotal === 0) { alert('Agregá al menos un producto'); return; }
    const cot = {
      id: uid(),
      fecha: new Date().toISOString(),
      nombre: nombreEnvio.trim() || `Cotización ${new Date().toLocaleDateString('es-AR')}`,
      fx, envio, aduana, productos, resultados,
    };
  ...
  const exportarCSV = useCallback(() => {
    if (resultados.productosCalc.length === 0) { alert('Nada para exportar'); return; }
    const nombre = nombreEnvio.trim() || 'cotizacion';
    const header = ['Producto', 'Cantidad', 'Precio unit CNY', 'Peso unit g', 'Link', 'Costo unit USD', 'Venta USD', 'Ganancia c/u USD', 'Ganancia total USD'];
    const rows = resultados.productosCalc.map(p => [
      p.nombre, p.cantidad, p.precioCNY, p.pesoG, p.link || '',
      p.costoUnitUSD.toFixed(2), p.ventaUSD.toFixed(2), p.gananciaUnitUSD.toFixed(2), p.gananciaTotalUSD.toFixed(2)
    ]);
    const totales = ['TOTAL', '', '', resultados.pesoTotalG, '', '', resultados.costoTotalUSD.toFixed(2), resultados.ingresoTotalUSD.toFixed(2), '', resultados.gananciaTotalUSD.toFixed(2)];
    const csv = [header, ...rows, totales].map(row => row.map(cell => {
  ...
  const limpiar = useCallback(() => {
    ...
    setNombreEnvio('');
    setProductos([]);
    setEnvio({ freightCNY: 0, serviceCNY: 0, recargaPct: 0.03, recargaFijo: 0.30, platformFee: 0.35, markup: 2.0 });
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 space-y-5">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <label className="block text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2">Nombre del envío</label>
          <input
            type="text"
            value={nombreEnvio}
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={actualizarDolar} className="text-xs bg-secondary hover:bg-secondary/80 text-secondary-foreground px-2 py-1 rounded-md transition-colors">
              ↻ Actualizar API
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              {resultados.pesoTotalG > 0 && (
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${resultados.pesoTotalG > 5999 ? 'bg-destructive/10 text-destructive' : 'bg-emerald-500/10 text-emerald-400'}`}>
                  {resultados.pesoTotalG}g / 5999g
                </span>
              )}
              <button onClick={addProducto} className="flex items-center gap-1 text-xs bg-primary text-primary-foreground px-2 py-1.5 rounded-lg hover:bg-primary/90 transition-colors">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">Producto</th>
                  <th className="text-right py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider w-24">¥ c/u</th>
                  <th className="text-right py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider w-20">Envío ¥</th>
                  <th className="text-right py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider w-20">g c/u</th>
                  <th className="text-right py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider w-16">Cant.</th>
                  <th className="text-right py-2 px-2 text-xs font-medium text-muted-foreground uppercase tracking-wider w-28">Venta USD</th>
                ...
                {productos.map(p => (
                  <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/20 transition-colors">
                    <td className="py-2 px-2">
                      <div className="flex items-center gap-2">
                        {p.imgURL && <img src={p.imgURL} alt="" className="w-8 h-8 rounded object-cover bg-muted flex-shrink-0" />}
                        <input
                          type="text"
                          value={p.nombre}
                    ...
                    <td className="py-2 px-2">
                      <div className="text-right">
                        {(p.envioLocalCNY > 0 || p.envioChinaCNY > 0) ? (
                          <span className="text-xs text-emerald-400" title={`Local: ¥${p.envioLocalCNY}, China: ¥${p.envioChinaCNY}`}>
                    ...
                    <td className="py-2 px-2">
                      <button onClick={() => removeProducto(p.id)} className="text-muted-foreground hover:text-destructive transition-colors">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="mb-4 p-3 bg-primary/5 border border-primary/20 rounded-lg">
            ...
            {lineaEnvio !== 'custom' && pesoTotalG > 0 && (
              <p className="text-xs text-muted-foreground mt-2">
                {pesoTotalG}g × {tarifaPorGramo.toFixed(3)} ¥/g = <span className="text-emerald-400 font-medium">¥{freightCalculado}</span>
              </p>
            )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              { label: 'Recarga fee (%)', value: envio.recargaPct * 100, key: 'recargaPct' as const, step: 0.01, transform: (v: number) => v / 100 },
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setAduana(prev => ({ ...prev, valorDeclaradoUSD: parseFloat(e.target.value) || null }))}
                  placeholder="auto"
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={guardarCotizacion} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">
            <Save className="w-4 h-4" /> Guardar cotización
          </button>
          <button onClick={exportarCSV} className="flex items-center gap-2 bg-secondary text-secondary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-secondary/80 transition-colors">
            <Download className="w-4 h-4" /> Exportar CSV
          </button>
          <button onClick={limpiar} className="flex items-center gap-2 bg-secondary text-secondary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-secondary/80 transition-colors">
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="space-y-2 text-sm">
          ...
          <Row label={`Recarga fee (${(r.recargaFee / (r.productosUSDTotal + r.freightUSD + r.serviceUSD) * 100).toFixed(0)}%)`} value={fmtUSD(r.recargaFee)} />
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="space-y-2 text-sm">
          ...
          {r.ahorroSubdeclaracionUSD > 0.01 && (
            <Row label="Ahorro subdeclaración" value={fmtUSD(r.ahorroSubdeclaracionUSD)} valueClass="text-emerald-400" />
          )}
            ...
            <div key={p.id} className="p-3 bg-secondary/30 rounded-lg">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`tabular-nums font-medium ${p.gananciaUnitUSD >= 0 ? 'text-emerald-400' : 'text-destructive'}`}>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${r.gananciaTotalUSD >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-destructive/10 text-destructive'}`}>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <div className={`p-3 rounded-lg ${r.gananciaTotalUSD >= 0 ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-destructive/10 border border-destructive/20'}`}>
            <p className={`text-xs font-medium uppercase tracking-wider mb-1 ${r.gananciaTotalUSD >= 0 ? 'text-emerald-400' : 'text-destructive'}`}>Ganancia neta</p>
            <p className={`text-xl font-bold tabular-nums ${r.gananciaTotalUSD >= 0 ? 'text-emerald-400' : 'text-destructive'}`}>{fmtUSD(r.gananciaTotalUSD)}</p>
            <p className="text-xs text-muted-foreground mt-1">{fmtARS(r.gananciaTotalARS)} · {fmtPct(r.margenTotalPct)}</p>
          </div>
        </div>
