...
export const fmtNumber = (n: number) =>
  isFinite(n) ? n.toLocaleString("es-AR", { maximumFractionDigits: 2 }) : "—";

export function uid() {
  return Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
}

export function calcularTodo(productos: Product[], fx: FxRates, envio: ShipmentCosts, aduana: AduanaConfig): CalculationResult {
  ...
  const fobDeclaradoUSD =
    aduana.valorDeclaradoUSD != null && aduana.valorDeclaradoUSD > 0 ? aduana.valorDeclaradoUSD : fobRealUSD;
  ...
  if (pesoTotalG > 5999) {
    alerts.push({
      type: "warning",
      msg: `Peso total ${pesoTotalG}g supera el máximo de 5999g. Partí el envío en dos paquetes.`,
    });
  }
  ...
  if (aduana.dentroFranquicia && aduana.enviosAnio >= 5) {
    alerts.push({
      type: "danger",
      msg: `Ya hiciste ${aduana.enviosAnio} envíos este año. La franquicia de $50 no aplica.`,
    });
  }
  if (aduana.dentroFranquicia && fobUSD > 50) {
    alerts.push({
      type: "warning",
      msg: `FOB declarado USD ${fobUSD.toFixed(2)} supera los $50. La franquicia no aplicaría.`,
    });
  }
  if (ahorroSubdeclaracionUSD > 1) {
    ...
    alerts.push({
      type: "warning",
      msg: `Estás declarando ${pct}% menos que el costo real. Ahorrás ${fmtUSD(ahorroSubdeclaracionUSD)} en impuestos pero es tu riesgo si Aduana abre el paquete.`,
    });
  }
